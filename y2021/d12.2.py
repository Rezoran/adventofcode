...
def recursionWorker(cavedict, startingpoint, path, smallcnt):
    global waycounter
    ...
    for elem in cavedict[startingpoint].adjacent:
        if elem.letter != "start":
            if elem.upper == True:
                recursionWorker(cavedict, elem.letter, path+cavedict[startingpoint].letter+"-", smallcnt)            
            elif (elem.upper != True and elem.letter not in path):
                recursionWorker(cavedict, elem.letter, path+cavedict[startingpoint].letter+"-", smallcnt)
            elif (elem.upper != True and smallcnt == 0):
                recursionWorker(cavedict, elem.letter, path+cavedict[startingpoint].letter+"-", 1)

    #print("dying path: {} from {}".format(path, startingpoint))
