dummyfile = """dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
...
exdata = """be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
def sumSegments(summand1: str, summand2: str) -> str:
    result = summand2
    for char in summand1:
        if char not in result:
            result += char
    #print("s1: {}, s2: {}, result: {}".format(summand1, summand2, result))
    return result

def subSegments(sub1: str, sub2: str) -> str:
    result = ""
    for char in sub1:
        if char not in sub2:
            result += char
    #print("s1: {}, s2: {}, result: {}".format(sub1, sub2, result))
    return result

def compareSegments(c1: str, c2: str) -> bool:
    #print("comparing {} and {} ->".format(c1, c2), end="")
    if len(c1) != len(c2):
    #    print("nope")
        return False
    for char in c1:
...
try:
    for line in dummyfile.splitlines():
        ...
        for element in samples.split(" "):
            length = len(element)
            #print("el: {}, len: {}".format(element, length))
            if length == 2:
                numbers[1] = element
            if length == 3:
                numbers[7] = element
            ...
            if length == 7:
                numbers[8] = element
            if length == 5:
                lengthOf5.append(element)
            if length == 6:
                lengthOf6.append(element)
        #print(numbers)
        # find 9
        for elem in lengthOf6:
            if len(subSegments(elem, numbers[4])) == 2:
                numbers[9] = elem
                break
        # find 2
        for elem in lengthOf5:
            if len(subSegments(elem, numbers[9])) == 1:
                numbers[2] = elem
                break
        # find 3 
        for elem in lengthOf5:
            if len(subSegments(elem, numbers[2])) == 1:
                numbers[3] = elem
                break
        #find 5 
        for elem in lengthOf5:
            if len(subSegments(elem, numbers[2])) == 2:
                numbers[5] = elem
                break
        # find 0
        for elem in lengthOf6:
            if len(subSegments(elem, numbers[5])) == 2:
                numbers[0] = elem
                break
        # find 6
        for elem in lengthOf6:
            if elem != numbers[9]:
                if len(subSegments(elem, numbers[5])) == 1:
                    numbers[6] = elem
                    break
        #print(numbers)
        for element in data.split(" "):
            for number in range(len(numbers)):
                if compareSegments(element, numbers[number]) is True:
                    lineresult += str(number)
        print("result of line: ", lineresult)
